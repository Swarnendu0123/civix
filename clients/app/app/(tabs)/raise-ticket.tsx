...
export default function Raiseticketscreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!location) {
      Alert.alert('Error', 'Location is required. Please allow location access or try again.');
      return;
    }
    ...
    try {
      ...
      if (!currentUser) {
        ...
        try {
          ...
          Alert.alert('Error', 'Could not create user account. Please try again.');
          ...
          <View style={styles.urgencyContainer}>
            {urgencyLevels.map((urgency) => (
              <TouchableOpacity
                ...
                onPress={() => setSelectedUrgency(urgency.value as 'critical' | 'moderate' | 'low')}
              >
                <View style={[
                  styles.radioButton,
                  selectedUrgency === urgency.value && { borderColor: urgency.color }
                ]}>
                  {selectedUrgency === urgency.value && (
                    <View style={[styles.radioButtonInner, { backgroundColor: urgency.color }]} />
                  )}
                </View>
                <Text style={[
                  styles.urgencyText,
                  selectedUrgency === urgency.value && { color: urgency.color, fontWeight: '600' }
                ]}>
                  {urgency.label}
                </Text>
              </TouchableOpacity>
